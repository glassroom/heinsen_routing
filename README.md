...
Moreover, we find evidence that our routing algorithm, as we apply it to the smallNORB visual recognition task, _learns to perform a form of "reverse graphics."_ The following visualization, from the paper, shows a 2-dimensional approximation of the manner in which pose vectors of an activated class capsule change as viewpoint elevation of the same object varies from one image to the next:
